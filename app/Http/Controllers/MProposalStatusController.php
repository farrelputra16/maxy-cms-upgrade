...
class MProposalStatusController extends Controller
{
    function getProposalStatus(){
        ...
        return view('m_proposal_status.indexv3');
    }

    function getProposalStatusData(Request $request){
        ...
        $mProposalStatus = MProposalStatus::select('id', 'name', 'description', 'created_at', 'created_id', 'updated_at', 'updated_id', 'status')
        ...
        foreach ($columns as $column) {
            ...
            if (empty($columnSearchValue) || in_array($columnName, ['DT_RowIndex', 'action'])) {
                ...
                if ($columnSearchValue == 'active')
                    $mProposalStatus->where('status', '=', 1);
                else
                    $mProposalStatus->where('status', '=', 0);
            } else {
                $mProposalStatus->where($columnName, 'like', "%{$columnSearchValue}%");
            }
        }

        return DataTables::of($mProposalStatus)
            ...
            ->addColumn('action', function ($row) {
                return '<a href="' . route('getEditProposalStatus', ['id' => $row->id, 'access' => 'm_proposal_Status_update']) . '"
