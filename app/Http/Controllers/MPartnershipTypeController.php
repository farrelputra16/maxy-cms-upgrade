...
class MPartnershipTypeController extends Controller
{
    function getPartnershipType(){
        ...
        return view('m_partnership_type.indexv3');
    }

    function getPartnershipTypeData(Request $request){
        ...
        $mPartnershipType = MPartnershipType::select('id', 'name', 'description', 'created_at', 'created_id', 'updated_at', 'updated_id', 'status')
        ...
        foreach ($columns as $column) {
            ...
            if (empty($columnSearchValue) || in_array($columnName, ['DT_RowIndex', 'action'])) {
                ...
                if (strpos(strtolower($columnSearchValue), 'non') !== false)
                    $mPartnershipType->where('status', '=', 0);
                else
                    $mPartnershipType->where('status', '=', 1);
            } else {
                $mPartnershipType->where($columnName, 'like', "%{$columnSearchValue}%");
            }
        }

        return DataTables::of($mPartnershipType)
            ...
            ->addColumn('status', function ($row) {
                return '<button 
                    ...
                    data-model="MPartnershipType">
                    ' . ($row->status == 1 ? 'Aktif' : 'Non aktif') . '
                </button>';
            })
            ->addColumn('action', function ($row) {
                return '<a href="' . route('getEditPartnershipType', ['id' => $row->id, 'access' => 'm_Partnership_type_update']) . '" 
