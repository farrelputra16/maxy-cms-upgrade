...
@section('title', 'Course Class Member Log')
@section('content')
                        ...
                        <div class="navbar-nav" style="margin-right: 10px;">
                            ...
                            @if (isset($_GET['course_name']))
                                <select class="ui dropdown" name="course_name">
                                    <option value="all" {{ $_GET['course_name'] == 'all' ? 'selected' : '' }}>all
                                    </option>
                                    @foreach ($course_name as $name)
                                        <option value="{{ $name->name }}"
                                            {{ $_GET['course_name'] == $name->name ? 'selected' : '' }}>
                                            {{ $name->name }}</option>
                                    @endforeach
                                </select>
                            @else
                        ...
                        <div class="navbar-nav">
                            ...
                            @if (isset($_GET['user_name']))
                                <select class="ui dropdown" name="user_name">
                                    <option value="all" {{ $_GET['user_name'] == 'all' ? 'selected' : '' }}>all
                                    </option>
                                    @foreach ($user_name as $userName)
                                        <option value="{{ $userName }}"
                                            {{ $_GET['user_name'] == $userName ? 'selected' : '' }}>{{ $userName }}
                                        </option>
                                    @endforeach
                                </select>
                            @else
                ...
                <div class="settings-container" style="margin-top: 2%; text-align: right">
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu" aria-labelledby="addColumnDropdown">
                            ...
                            <label class="dropdown-item">
                                <input type="checkbox" class="column-checkbox" data-column="ID Course Class Module"> ID
                                Course Class Module
                            </label>
                            <label class="dropdown-item">
                            ...
                            <td>
                                @if ($item->status_log == 1)
                                    @if ($item->course_type == 'pretest' || $item->course_type == 'postest' || $item->course_type == 'unjukketerampilan')
                                        {{ $item->user_name }} di kelas {{ $item->course_name }} - Batch
                                        {{ $item->batch }} Mendapatkan Paket Soal {{ $item->paket_soal }} Mengerjakan
                                        Module yaitu {{ $item->course_module_name }} -Day {{ $item->day }}
                                    @elseif($item->course_type == 'assignment')
                                        {{ $item->user_name }} di kelas {{ $item->course_name }} - Batch
                                        {{ $item->batch }} Mengumpulkan(submit) Tugas Module yaitu
                                        {{ $item->course_module_name }} - Day {{ $item->day }}
                                    @endif
                                @elseif($item->status_log == 2)
                                    ...
                                    @else
                                        {{ $item->user_name }} di kelas {{ $item->course_name }} - Batch
                                        {{ $item->batch }}, Membuka Module yaitu {{ $item->course_module_name }} - Day
                                        {{ $item->day }}
                                    @endif
                                ...
                                @elseif($item->status_log == 4)
                                    ...
                                    @else
                                        {{ $item->user_name }} di kelas {{ $item->course_name }} - Batch
                                        {{ $item->batch }}, Menghapus(unsubmit) Tugas Modulenya yaitu
                                        {{ $item->course_module_name }} - Day {{ $item->day }}
                                    @endif
                                @else
                                    {{ '' }}
