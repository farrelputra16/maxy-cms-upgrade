...
class MClassTypeController extends Controller
{
    public function getClassType() {
        ...
        return view('m_class_type.index');
    }

    public function getClassTypeData(Request $request) {
        ...
        $mClassType = MClassType::select('id', 'name', 'description', 'created_at', 'created_id', 'updated_at', 'updated_id', 'status')
        ...
        foreach ($columns as $column) {
            ...
            if (empty($columnSearchValue) || in_array($columnName, ['DT_RowIndex', 'action'])) {
                ...
                if ($columnSearchValue == 'active')
                    $mClassType->where('status', '=', 1);
                else
                    $mClassType->where('status', '=', 0);
            } else {
                $mClassType->where($columnName, 'like', "%{$columnSearchValue}%");
            }
        }

        return DataTables::of($mClassType)
            ...
            ->addColumn('status', function ($row) {
                return '<button
                    ...
                    data-model="MClassType">
                    ' . ($row->status == 1 ? 'Aktif' : 'Non aktif') . '
                </button>';
            })
            ->addColumn('action', function ($row) {
                return '<a href="' . route('getEditClassType', ['id' => $row->id, 'access' => 'm_class_type_update']) . '"
