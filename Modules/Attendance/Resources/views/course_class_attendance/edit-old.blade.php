...
@section('content')
    ...
    <div class="container">
        <form class="ui form" action="{{ route('postEditCourseClassAttendance') }}" method="post">
            @csrf
            ...
            <div class="field">
                ...
                <input type="text" name="name" placeholder="Masukkan Nama" value="{{ $attendance->name }}">
            </div>
            <div class="field">
                <label for="">Day</label>
                <select class="form-control select2 w-100" name="day" data-placeholder="Choose ...">
                    @foreach ($class->parent_modules as $item)
                        <option value="{{ $item->id }}" @if ($item->id == $attendance->course_class_module_id) selected @endif> Day
            ...
            <div class="field">
                <div class="ui checkbox">
                    <input class="form-check-input" type="checkbox" value="1"
                        {{ $attendance->status == 1 ? 'checked' : '' }} name="status">
                    <label>Aktif</label>
                </div>
            </div>
            ...
            <div class="divAdd">
                ...
                <button class="btn btn-primary">Tambahkan Absensi</button>
            </div>
        </form>
